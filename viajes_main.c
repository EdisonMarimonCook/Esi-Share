...
void buscar_viaje(estruct_viajes *v, int lineas){  //ACEPTA VIAJES QUE NO SE ENSEÑAN X PANTALLA

    //Declaración de datos:
    FILE *f;
    int i, encontrado=0, id=0, bucle=0;
    char vector[N], aux;
    ...
    while(bucle==0){
        ...
        i--; //Porque después coge la línea siguiente
        ...
        if(encontrado==0){
            ...
            if(strcmp(v[i].Estado,"ABIERTO")==0){
                ...
                printf("%d-%s-%s-%s-%s-%d-%d-%s\n", v[i].Id_viaje, v[i].Id_mat, v[i].F_inic, v[i].H_inic, v[i].H_fin, v[i].Plazas_libre, v[i].Importe, v[i].Estado);
...
void subir_viaje(estruct_viajes *v, int lineas, estruct_vehiculos *vec, int l){  //Hecho
...
void eliminar_viaje(estruct_viajes *v, int lineas){  //Hecho
    ...
    FILE *viajes_file_aux, *viajes_file, *pasos, *pasos_aux;
    ...
    pasos_aux=fopen("pasos_aux.txt","w+");
    if(pasos_aux==NULL){
        error();
    }
    ...
    //Lo mismo pero para pasos:
    while(!feof(pasos)){
        leer(vector_temporal,pasos);
        id_aux=atoi(vector_temporal);
        if(id!=id_aux){
            fputs(vector_temporal,pasos_aux);
            fputc('-', pasos_aux);
            fgets(vector_temporal,N,pasos);
            fputs(vector_temporal,pasos_aux);
        }else{
            fgets(vector_temporal,N,pasos);
        }
    ...
    //Aquí borro el viaje en pasos tmb:
    system("rename pasos.txt borrador.txt");
    system("rename pasos_aux.txt pasos.txt");
    system("del borrador.txt");
